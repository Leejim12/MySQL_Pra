...
-- 11.<수강신청> 테이블에서 '날짜'가 '2018-01-01’ 이전에 수강 신청한 레코드의 '수강신청번호', 학번', '날짜'를
-- 읽어오는 SQL문을 작성하시오.
...
-- 20 <수강신청> 테이블에서 '1601001' 학생을 제외한 모든 학생의 수강신청 데이터를 가져오는 SQL문을
-- 작성하시오
...
-- 30 <과목> 테이블에서 '영문명'이 'T'로 시작하여 'n'으로 끝나는 과목의 '과목번호, ’과목명', '영문명'을 가져오는
-- SQL문을 작성하시오.
select 과목번호,과목명,영문명 from 과목 where 영문명 like 'T%n';

-- 31 <과목> 테이블에서 '과목명'이 ‘컴퓨터ㅇㅇ'와 같은 이름을 가진 과목의 '과목번호', '과목', '영문명'을 가져오는
-- SQL문을 작성하시오.
select 과목번호,과목명,영문명 from 과목 where 과목명 like '컴퓨터%';
...
select 학번, 이름, left(이름,1) as 성씨 from 학생;
...
select 담당교수,과목명,영문명 from 과목;

-- 43 <과목> 테이블에서 '과목명'과 화점', 그리고 한 학기를 15주라 가정했을 때 한 학기 총 수업 시간 수를
-- 읽어오는 SQL문을 작성하시오.
select 과목명,학점, 시수*15 as '한 학기 총 수업 시간 수' from 과목;
...
-- 55 <수강신청내역> 테이블에서 아래와 같이 과목별로 수강자 수를 읽어오는(과목번호, 수강자 수) SQL문을
-- 작성하시오.
